class Workshift < ActiveRecord::Base
  ...
  validate                    :no_busy_angel,         :on => :create
  validate                    :must_have_tickets,     :on => :create
  validate                    :cashbox_availability,  :on => :create

  scope :in_progress, where("state != 'cleared'").order("created_at ASC")
  scope :cleared,     where("state = 'cleared'").order("created_at ASC")
  ...
  private
  def no_busy_angel
    errors.add(:base, "The user you chose is busy") if user && user.workshift
  end
  ...
  def must_have_tickets
    ...
    if ticket_amount == 0
      errors.add(:base, "You have not supplied any tickets")
    end
  end

  def cashbox_availability
    if Cashbox.busy.include? self.cashbox
      errors.add(:base, "Cashbox is already in use by other Workshift")
    end
  end
end
